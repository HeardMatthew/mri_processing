...
%   functions -- MH
% 28/02/18  Running new sub-analysis to look at which acquisition time
...
for rr = 1:length(hybridRuns)
    ...
    % File names
    [boldFiles, ~] = spm_select('List', GLMfuncDir, ['^swuhybrid_run' num2str(rr) '_00*.*\.nii$']);
...
designDir = fullfile(designRootDir, 'hybrid');
...
for rr = 1:length(isssRuns)
    % Extract names for all bold files and put into job struct
    % File names
    [boldFiles, ~] = spm_select('List', GLMfuncDir, ['^swuisss_run' num2str(rr) '_00*.*\.nii$']);
...
designDir = fullfile(designRootDir, 'isss');
matlabbatch{1}.spm.stats.fmri_spec.dir = {designDir};
